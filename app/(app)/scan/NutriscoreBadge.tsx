...
export default function NutriscoreBadge({ grade }: { grade: string }) {
  const g = grade.toLowerCase();
  const color = COLORS[g] ?? "bg-muted-2 text-white";

  return (
    <span
      className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[11px] font-bold uppercase ${color}`}
